...
|------|----------|------------|------|
| **① 发模板** | 优先复制 `prompts/STANDARD_PROJECT_PROMPTS.md` 中最接近当前任务的模板；拿不准时直接用“通用总模板” | 不要自己随意写开场白，不要删掉模板里的协议骨架 | 模板已包含当前项目所需的读档、计划、验证、落库与收口要求 |
...
| 换了新对话框，需要快速衔接 | [1.1 标准新对话模板](#11-标准新对话模板) |
| 当前对话快结束了，需要存档 | [1.2 上下文保存机制](#12-上下文保存机制对话结束前) |
| 需要 AI 直接推送代码到 GitHub | [1.3 GitHub 持久认证与远程校验](#13-github-持久认证与远程校验) |
| 想确认 AI 说的“已推送”是不是真的已上远程 | [1.3 GitHub 持久认证与远程校验](#13-github-持久认证与远程校验) |
...
**核心设计**：AI 每次新对话先读 `SESSION_TRACKER.md`，并遵守 `docs/STANDARD_CONVERSATION_PROTOCOL.md`；随后再按任务类型补读专项文档。当前 `prompts/STANDARD_PROJECT_PROMPTS.md` 已经把 **仓库地址、默认执行路径 `/home/ubuntu/MarioTrickster`、默认分支 `master`、必读文档入口与 push 后远程 SHA 校验要求** 固化进模板骨架，所以一般续接场景下，你开新对话时通常**不需要手动复制粘贴任何长内容，也不需要重复填写这些稳定信息**；原则上只需补充：**本次任务 / 本次反馈**，必要时再补【我的补充要求】。但如果是**换号 / 换窗口后的新对话读档续接**，为了避免 AI 读档后在提交、推送或远程 SHA 校验阶段中途停下，开场模板里应**一并附上可用 token**。

### 1.1 标准新对话模板

> **默认规则**：以后所有“更新 / 升级 / 优化 / 续接”类新对话，优先使用 `prompts/STANDARD_PROJECT_PROMPTS.md`；如果你懒得区分场景，就直接用其中的 **通用总模板**。
>
> **配套协议**：模板的固定骨架受 `docs/STANDARD_CONVERSATION_PROTOCOL.md` 约束。你的个性化问题可以继续提，但请统一放在模板末尾的【我的补充要求】里，不要替换掉协议段落。
...
### 1.3 GitHub 持久认证与远程校验

**为什么要升级这一段？** 过去只要“本地提交成功但远程未验证”，就可能出现**AI 以为已经推送，实际上改动还停留在本地**的问题。现在这个项目把 GitHub 推送流程升级为两层保障：**认证尽量持久化**，以及**每次推送后强制校验远程提交号**。

#### 1.3.1 当前项目的默认规则

| 规则 | 要求 | 目的 |
| :--- | :--- | :--- |
| 认证规则 | 当前环境已配置可复用的 Git 凭据；若 token 过期、被撤销或环境重置，则重新提供新 token | 减少每次重复索要 token 的概率 |
| 推送规则 | AI 只要做了会写仓库的改动，就必须尝试提交并推送 | 避免成果长期滞留在本地 |
| 校验规则 | **push 之后必须再查一次远程分支 commit SHA**，并与本地 `HEAD` 比对 | 防止“口头说已推送，实际上未上远程” |
| 交付规则 | 最终汇报里必须写明：分支、提交信息、短 SHA、远程校验结果、工作区是否 clean | 让你一眼知道是否真正同步完成 |
| 失败规则 | 只要远程校验不通过，就必须明确标注为“仅本地完成，远程未同步” | 杜绝误判 |

#### 1.3.2 什么时候还需要重新给 token？

> **并不是以后永远都不用再给。** 如果出现 token 过期、你主动撤销 token、GitHub 权限变化，或运行环境被重置，AI 仍然可能需要新的 token。

判断标准非常简单：如果 AI 无法完成 `git push` 或无法验证远程分支 commit，就说明当前认证不可用，这时再补新的 token，而不是默认认为远程已经更新。

#### 1.3.3 标准生成方式（需要重新补 token 时使用）
...
6. 点击 **Generate token**
7. 复制生成的 `ghp_xxxx` 字符串，妥善保存

#### 1.3.4 以后凡是“需要推送”的对话，AI 必须执行的收口动作
...
只有当**本地 `HEAD` 与远程 `master` 的 commit SHA 一致**时，才能把任务汇报为“已推送完成”。否则必须汇报为：

```text
当前成果仅完成本地提交 / 本地写入，远程 GitHub 尚未同步成功。
```

#### 1.3.5 最终汇报固定格式（以后默认执行）

以后凡是涉及仓库写入、文档更新、代码修改、提示词维护、协议改造的任务，结果里至少要包含下表：

| 字段 | 必填内容 |
| :--- | :--- |
| 分支 | 例如 `master` |
| 最新提交 | 例如 `abc1234` |
| 提交信息 | 本次 commit message |
| 远程校验 | 远程分支 SHA 是否与本地一致 |
| 工作区状态 | 是否 `working tree clean` |
| 失败说明 | 若未同步成功，写明卡在认证、网络还是权限 |
...
| 到期后重新生成 | 到期后重新配置即可 |
| 可随时撤销 | 在 GitHub 设置里删除即可立即失效 |
| 不以“已 commit”代替“已推送” | 只有远程校验成功才算真正完成同步 |
...
2. 更新 SESSION_TRACKER.md、Progress_Summary.md 和 MASTER_TRACKER.md 并推送
3. 停止其他工作
```
...
| **新对话读档带 Token** | 换号 / 换窗口后的新对话续接，开场尽量一并附上 GitHub Token | 让 AI 读档后若需要提交、推送或远程校验时不被中途打断 |
...
| 换号 / 换窗口的新对话读档不带 Token | 这类续接优先使用标准续接模板，并在开场一并附上可用 Token |
...
> **核心理念**：把 AI 看作一位**有经验但看不见你屏幕的远程结对编程伙伴**。你提供的"情报"越精确，AI 输出的代码质量就越高，消耗的积分就越少。一般续接时，重点补 **任务** 与 **反馈**；而在**换号 / 换窗口的新对话读档交接**里，则优先用好标准模板并一并附上 Token，让交接尽量零损耗。
