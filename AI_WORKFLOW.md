...
**核心设计**：AI 每次新对话先读 `SESSION_TRACKER.md`（当前状态 + AI 行为规范 + 回归清单 + 待办队列），需要完整上下文时再读 `MarioTrickster_Progress_Summary.md`（功能清单、Bug 库、技术决策、Session 历史）。你开新对话时**不需要手动复制粘贴任何长内容**，只需要给 AI 三样东西：Token、仓库地址、本次任务。
...
修复 Bug 时修改的代码可能影响其他已通过或未测试的功能。项目已建立了一套“影响矩阵 + 定向回归”机制来解决这个问题：

**你不需要做任何额外工作**，AI 会自动处理：

1. AI 修复 Bug 后，自动查询影响矩阵（详见 `Testing_Guide.md` 第四章）
2. AI 在 `SESSION_TRACKER.md` 第 3 节“回归验证清单”中标记需要重测的项目
3. 你测试时，除了当前测试项，还需快速验证清单中的回归项（每项约 30 秒）
4. 在反馈模板中一起填写结果

这样既不需要全量回归（节省时间），也不会遗漏影响（AI 自动分析）。
