...
public class TestSceneBuilder : Editor
{
    ...
    public static void BuildTestScene()
    {
        ...
        CreateStageSign(s7 + STAGE_WIDTH / 2f, "STAGE 7: WIN/LOSE & UI",
            "Mario reach GoalZone -> Mario Wins\nMario fall to death -> Trickster Wins\nR/F5 = Fresh Retry | N = Next Round\nCheck: UGUI text/font, elapsed time, outcome reason\nAbility feedback: text + backdrop fade together, including pause\nAI: mechanism probes OR 3 decision rooms / 9 matchups\nCheck actual routes, telegraph retreat, recovery crossing, loot return\nB: land on top to launch; side contact must not launch\nCheck report track: Smoke is NOT Experience; requests are not launch events\nS158: jump THROUGH one-way decks, land on each step\nUpper completion and anchor changes must be actual, not requests\nS178: TWO MOUTHS playable loop / ordinary inputs\nVisible source -> cost decision -> real fork retreat -> complete route\nGuard sees within 12 units; memory expires after 2.5 seconds\nNo hidden route/loot input; holding and relocating can fail\nNew map plus AI, not old-map causal A/B or verified fun\nS177: research-backed design review, not learned AI or a fun score\nPassive route first; then pressure, cue, input, return and human feedback\nOne hypothesis, one variable, a falsifier; S176 gameplay frozen\nS176: export full feedback FIRST; optional demos are new runs\nSolid-step descent: ordinary walk-off input, not a completed landing\nSame map/wall strategy; terrain, cooldowns and assertions unchanged\nS175: same cave A/B policy comparison - 12 first runs, one feedback ZIP\nVisible warning/solid/reopen -> bounded input -> measured crossing\nNo cue is not no opportunity; requests are not bait or damage prevention\nThree development reviews are NOT three verified Unity duel rounds\nS174: new cavern experiment - solid roofs, vent shaft, surface lookouts\nLower AI remembers actual scan/retreat and may choose upper return\nSix comparisons -> inspect actual routes/return pressure -> export ZIP\nOld maps remain unchanged; no fun score or automatic new-mechanic code\nS173: compact import paths, check staging/final/metadata/ZIP length before writing\nRe-import old ZIP into short folders; never bypass failed regression\nS172: import single or multi-report ZIP without regenerating maps\nImported history is read-only, deduplicated and never auto-resumed\nSelect exact version/variant/mode; latest is not best\nS171: report actions come BEFORE collapsed technical details\nWatch or play the saved report map without regenerating the draft\nFirst-pass and confirmation stay separate; two variants still stop\nKnown encounters do not certify fun, lower tunnels or return pressure\nS170: six comparisons include BOTH routes; toolbar is single-play only\nPreparation != timely interception; landed ascent/descent, max two requests\nFallen stair recovery rewinds real landings, never skips failed targets\nFirst-pass clears cannot hide a surface confirmation fallback\nS169: feedback completeness is NOT a fun verdict\nDo not plan a tunnel from a temporary braking speed\nArrival != re-blended ready; 3-second controls != later return residence\nCompare report-bound routes and preserve a parent interaction lost in a child\nS168: Creation -> seeded duel -> play -> one connection variant\nActive tunnel opponent in demos; Passive is a no-interference baseline\nTravel + blend + telegraph must fit before interception\nSix real matchups before changing layout; at most two variants\nKeep parent results; neither more damage nor more clears means more fun\nS166: TunnelDuel = 3 hypotheses x 6 matchups\nTunnel requests != departures != actual arrivals\nHuman/demo records never certify automated coverage; export feedback ZIP\nS165: Adaptive/SafeRoute scan local evidence or blocker windup, not empty anchors\nOpponent prepares independently while Mario waits\nMissing/duplicate matchups cannot certify the batch\nClean crossing segment is NOT a clean whole encounter\nPublic queue state/countdown/reach; Idle is NOT safe\nCounterplay: matched timing and passive-opponent controls\nRead / wait / cross / damage are separate evidence, not a fun score\nFailed regressions stop the batch; inspect TestReport.txt\nExplorer: one target per type, 4s each, max 8s / half round\nExpired visits are NOT passes; timeouts get confirmation priority\nHammer contact is NOT damage; health loss has no source attribution\nSuspended hazard mounts are not required landings; real high platforms still checked\nDrop cancellation restores collision; spikes keep authored height\nScan hit/miss is separate from cast count; baiter prepares before close range\nPer-mechanism observations are not full behavioral passes",
            stage7.transform);
    ...
    {
        if (!EditorUtility.DisplayDialog(
            "MarioTrickster - 生成验证关卡",
            "这将生成一个覆盖完整游戏循环的验证关卡（已整合原 Commit0-6 与 Gameplay Loop）。\n\n" +
        ...
        EditorSceneManager.MarkSceneDirty(EditorSceneManager.GetActiveScene());
        Debug.Log("[TestSceneBuilder] Validation scene generated: integrated lane covers onboarding, hide/possess, route budget, combo heat, loot escape, scan crisis, Q reveal and final goal.");
        EditorUtility.DisplayDialog(
            "验证关卡已生成",
            "已整合原 Commit0-6 与 Gameplay Loop 为统一验证关卡：MarioTrickster → Build Validation Scene。\n\n" +
    ...
    private static void ClearTestSceneObjectsSilently()
    {
        Camera mainCam = Camera.main;
        if (mainCam != null)
        {
    ...
    private static GameObject CreateValidationBlock(string name, Vector3 position, Color color)
    {
        GameObject block = CreateSceneProp(name, position, color, new Vector3(1.2f, 1.2f, 1f));
        block.AddComponent<ControllableBlock>();
        PossessionAnchor anchor = block.AddComponent<PossessionAnchor>();
        SetSerializedField(anchor, "anchorId", name);
        return block;
    }

    private static GameObject CreateValidationPlatform(string name, Vector3 position, Color color)
    {
        GameObject platform = CreateSceneProp(name, position, color, new Vector3(2.4f, 0.5f, 1f));
        platform.AddComponent<ControllablePlatform>();
        PossessionAnchor anchor = platform.AddComponent<PossessionAnchor>();
        SetSerializedField(anchor, "anchorId", name);
        return platform;
    }
