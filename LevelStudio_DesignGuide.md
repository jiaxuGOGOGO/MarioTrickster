...
| `[` | 临时封路机关 (ControllableBlocker) | 半透明蓝 | `]` | 公开队列机关 (StateQueueTrap) | 橙红 |
| `.` | 空气 (Air) | 不生成 |  |  |  |
...
| S2_Duel_QueueTrap_01（公开队列机关） | Current/Next 公开信息 + 高代价跳状态 | S2 双路线博弈房 |
...
项目目前已具备非常扎实的基础框架，涵盖 26 种核心元素（均已在 `AsciiElementRegistry` 默认字典中注册）：

| 类别 | 元素 |
|------|------|
| 地形类 | 地面 `#`、平台 `=`、墙壁 `W`、单向平台 `-`、移动平台 `>` |
| 陷阱类 | 地刺 `^`、火焰陷阱 `~`、摆锤 `P`、临时封路机关 `[`、公开队列机关 `]` |
| 互动平台 | 弹跳平台 `B`、崩塌平台 `C` |
| 敌人类 | 简单巡逻敌人 `e`、弹跳怪 `E` |
| 隐藏与欺骗 | 伪装墙 `F`、隐藏通道 `H` |
