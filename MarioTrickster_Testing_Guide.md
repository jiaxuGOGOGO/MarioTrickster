...
| **BouncyPlatform** (弹跳平台) | Mario | 从任意方向碰撞平台 | 修正后法线方向弹射 + 完整抛物线(水平动能保留，非垂直下落) + 挤压动画 + 喜剧延迟 + 相机震动 [S21: SetFrameVelocity绝对注入+maxSpeed跳过] |
...
|------------|----------------|------------|------|
| MarioController.cs | 测试 1、6.5、9E | 测试 3、7 | 移动/跳跃改变影响平台跟随和终点触发。S21: SetFrameVelocity+maxSpeed跳过影响弹跳平台 |
