...
| 字段 | 值 |
|------|-----|
| **最新 Session** | Session 96（标准对话协议与标准提示词模板库落地） |
| **日期** | 2026-04-12 |
| **分支** | master |
| **阶段** | Sprint 2.5 关卡主线桥接期 + 美术落地治理收束 — 本轮新增 `docs/STANDARD_CONVERSATION_PROTOCOL.md` 与 `prompts/STANDARD_PROJECT_PROMPTS.md`，把后续所有“更新 / 升级 / 优化项目”的新对话统一改为**先按标准协议读档、判断阶段、给计划，再执行、验证与落库**；同时在 `README.md` 接入默认入口与模板索引。 |
| **编译状态** | ✅ 无代码编译变更；本次仅更新文档与 `SESSION_TRACKER.md`，用于固化新对话标准入口、提示词骨架与协作收口规则 |
| **阻塞** | ⏳ 项目级协作骨架已补强，但当前唯一应优先突破的实务瓶颈仍是 `MarioTrickster_Style_epoch_10.safetensors` 的本地验证结果缺失；**现在缺的不是新的泛研究，而是按新标准模板把既有验证入口真正执行掉，并把首批资产接回定义绑定到已命名需求上。** |
| **交接说明** | S96 已把“以后新对话必须优先使用标准提示词骨架，而不是随意自由发挥”的要求正式落库。默认顺序继续保持不变：**项目主线从白盒关卡窄入口续航，美术支线从 LoRA 本地验证入口启动，验证通过后再进入首批命名资产切片量产。** 后续如需开启项目级升级、白盒推进、LoRA 验证、教程蒸馏、Bug 修复或会话收口，优先从 `prompts/STANDARD_PROJECT_PROMPTS.md` 复制对应模板。 |
...
| 优先级 | 描述 | 状态 |
|--------|------|------|
| **最高** | **`trickster_style` 本地验证闭环**：用户已完成首个自然语言 LoRA 登记，当前主任务仍是验证 `MarioTrickster_Style_epoch_10.safetensors` 的触发词甜区、推荐权重、污染模式与专属去污词，并把结论回填到配方库名录。**S91 已固化默认作业模板，S95 已把它升级为当前阶段默认起步入口，S96 又把“所有新对话先套标准提示词骨架再执行该入口”的要求正式落库**：固定同底模/同 Seed，对 `静态资产 / 角色动作 / 横版场景` 三类题材执行 `0.6 / 0.8 / 1.0` 三档权重扫描，并保留“不挂 LoRA / 弱触发 / 正常触发”三组对照，记录风格命中率、轮廓稳定性、题材污染与专属去污词。 | 🚀 模板已落库，当前默认就从这一步开始执行并回传结果 |
...
| **高** | **批量资产生产**：概念锚点确立后，先用“探索 30 张炼 LoRA”的白话入口低成本验证方向，再积累满意样本训练全局 Style LoRA；LoRA 落地后必须先登记入册，再用“做一组某某资产”的白话入口批量出图，并用 `AI_SpriteSlicer` 一键切片。**S94 追加约束**：任何正式资产进入量产前，必须先补齐“目标槽位 / 目录位置 / 命名规则 / 导入参数 / 废弃条件”的接回定义。**S95 追加执行顺序**：默认先做首批命名资产的窄切片，不直接上大批量。 | ⏳ `trickster_style` 已登记，等待完成本地验证后，先从首批命名资产切片开始 |
