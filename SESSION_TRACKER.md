...
| 字段 | 值 |
|------|-----|
| **最新 Session** | Session 91 (固化 trickster_style 本地验证默认作业模板) |
| **日期** | 2026-04-12 |
| **分支** | master |
| **阶段** | Sprint 2 美术生产工作流 — 已在读档续接后把 `trickster_style` 的**本地验证默认作业模板**固定下来，后续再收到“LoRA 练完了，告诉我怎么验证”的白话请求时，不必重新猜测试顺序，可直接进入执行。 |
| **编译状态** | ✅ 无代码编译变更；本次仅更新 `SESSION_TRACKER.md`，用于沉淀可复用的本地验证流程 |
| **阻塞** | ⏳ 美术主线阻塞收敛为“只差本地执行”；`MarioTrickster_Style_epoch_10.safetensors` 的验证顺序已固定，但仍缺用户在本机按模板跑出的实测结果，用于回填触发词甜区、推荐权重与专属去污词 |
| **交接说明** | S91 起，`MarioTrickster_Style_epoch_10.safetensors` 的默认验证口径固定为：**同底模、同 Seed、同题材做 A/B 对照**，先跑 **静态资产 / 角色动作 / 横版场景** 三类代表题材，再做 **0.6 / 0.8 / 1.0** 三档权重扫描，并保留“**不挂 LoRA / 挂 LoRA 但弱化触发词 / 挂 LoRA 且正常触发**”三组对照。记录时只看四件事：**风格命中率、轮廓稳定性、题材污染、是否需要专属负面词**。若 0.8 附近已能稳定统一线稿与平涂、同时不把海景/红屋顶等训练题材带进无关资产，就优先记为推荐档；若高权重出现固定脏特征，则把症状与对应去污词一起回填到 LoRA 名录，再进入正式量产。与此同时，前台虽然可以继续压缩成大白话入口，但后台仍必须保留旧版高价值约束：**静态资产 / 动画序列 / 透视场景分流**、`LoRA / Prompt / ControlNet / KSampler` 四区隔离、`物理防滑步 > 空间透视正确 > 光影材质表现` 优先级，以及所有能减少返工的失败模式、参数甜区、负面禁区与锚点 Seed。 |
