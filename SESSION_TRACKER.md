...
| 字段 | 值 |
|------|-----|
| **最新 Session** | Session 85 (训练提问白话入口补丁) |
| **日期** | 2026-04-11 |
| **分支** | master |
| **阶段** | Sprint 2 美术生产工作流 — 已将 `docs/ART_PIPELINE_GUIDE.md` 进一步扩展为“训练提问白话入口增强版”，现在不只覆盖从零开始、喂书蒸馏、探索出 30 张炼 LoRA、LoRA 登记与正式量产，还补齐了本地训练参数、本地训练操作、Civitai 页面填写与训练排障的自然语言入口；`prompts/PROMPT_RECIPES.md` 顶部也已同步加入同类摘要。 |
| **编译状态** | ✅ 无代码编译变更；已更新 `prompts/PROMPT_RECIPES.md`、`docs/ART_PIPELINE_GUIDE.md` 与本文件，可直接提交推送 |
| **阻塞** | ⏳ 无流程性阻塞；下一步只等用户直接用“本地训练参数”“Civitai 页面填写”或“训练排障”类白话句式发起真实请求，以验证训练辅导入口在新会话中的稳定性 |
| **交接说明** | S85 起，前端白话模式已从“流程入口极简”进一步扩展到“训练求助极简”。用户现在除了可以说“我想从零开始做这套美术资产，你带我走”“我上传了一本教程，帮我蒸馏进配方库并推送仓库”“基于这个参考风格，给我一套本地跑图工单，我要连续抽 30 张拿去炼 LoRA”“我炼完 LoRA 了，文件名是 A，触发词是 B。帮我登记”或“做一组地刺的静态图”，还可以直接说“我要在本地训练这个 LoRA，直接给我一套能填的参数和操作顺序”“我要用 Civitai 训练这个 LoRA，直接告诉我各项怎么填”或“这是我现在的训练参数或截图，帮我直接检查哪里有坑”。系统必须后台静默把自然语言映射到训练建议、页面指导或排障流程，除非缺少致命执行条件，否则不得追问长表单。 |
...
| 优先级 | 描述 | 状态 |
|--------|------|------|
| **最高** | **训练提问白话入口首轮实测**：当前速查表已补齐“从零开始 / 喂书 / 探索 30 张炼 LoRA / 本地训练参数 / Civitai 页面填写 / 训练排障 / LoRA 登记 / 正式量产”八类自然语言入口。下一步只需等待用户直接用任一白话句式发起真实任务，系统即应后台静默映射到正确流程。 | 🚀 等待用户直接发起首条训练或生产白话任务 |
