...
### 2.1 菜单2别再“一次说满”：改成对话驱动补位

如果你现在只是想**先跑通测试**，那么在菜单2里最忌讳的事情，就是一上来逼自己把设定、画风、镜头、材质、动作全部一次讲完整。更高效的做法是：你只提供当前手里已经有的那部分信息，然后要求 Manus 反向告诉你，**现在缺的是哪几个槽位、下一句该怎么发、暂时不要碰哪些部分**。这样你就不会在菜单2、菜单2.5、菜单3之间来回猜模板，而是始终由 Manus 在每个节点告诉你“该换哪一格”。

从这一版开始，菜单2相关对话建议统一采用 **动态补位卡**。也就是说，Manus 在回复时，不再只给散文解释，而是必须明确标出你当前位于哪个菜单、已经锁定了哪些信息、还缺哪几个必要输入，以及你下一条可以直接复制出去的消息。这样你就能把“教程阅读”变成“管线驾驶”，边聊边推进。

| 动态补位卡区块 | Manus 必须输出什么 | 你的动作 |
|------|------|------|
| **当前位点** | 明确写出你当前处于 `菜单2-A 新实体入库`、`菜单2-B 画风抽卡`、`菜单2.5 换轨准备` 或 `菜单3 前置登记` 哪一档 | 先确认自己现在在哪一步，不再猜流程 |
| **已锁定槽位** | 已经确定的信息，例如实体类别、参考图已上传、目标风格方向、是否只是测试 | 你只核对，不重复描述 |
| **待补槽位** | 最多只列 1–3 个当前真的缺失的关键字段，例如“实体用途”“LoRA 文件名”“是否要做静态还是动画” | 你只补这些，不额外加戏 |
| **下一条可复制口令** | Manus 帮你把下一条消息拼好，并用 `【待你替换】` 标出需要你填的词 | 直接复制，替换括号内内容后发送 |
| **本地动作提醒** | 明确告诉你现在应该上传参考图、去本地抽卡、等待 LoRA、还是暂时不要动 ComfyUI | 防止你在错误菜单提前开跑 |

> **建议 Manus 的标准回复格式固定为：**
>
> `【当前位点】菜单2-B / 画风抽卡期`
>
> `【已锁定】已有 2 张参考图；目标为横版平台游戏；当前只是测试跑通流程。`
>
> `【待补槽位】1. 你更偏角色还是场景；2. 这轮先做静态还是动画。`
>
> `【请直接复制这条继续】Manus，继续菜单2-B。我这轮优先测试【待你替换：角色/场景】方向，并先做【待你替换：静态/动画】资产，请按最小可跑通方案给我下发。`
>
> `【你现在先别做】先不要自己写最终生图 Prompt，也不要提前切菜单3。`

### 💬 如果你现在只是想跑通测试，直接发这句

> **"Manus，我现在走测试模式。请不要假设我已经把信息说全。你先判定我当前处于菜单2的哪一步，再用【当前位点 / 已锁定 / 待补槽位 / 下一条可复制口令 / 本地动作提醒】五段式告诉我。我只补必要字段，不一次写满。"**
...
### 3.1 菜单2.5 → 菜单3 的接棒公式（升级为“触发表 + 换点协议”）

菜单2.5 到菜单3最容易卡住的地方，不是你不会说话，而是你不知道**现在到底该换哪个槽位**。因此，接棒期的核心目标不应只是“给一句换轨口令”，而应当是让 Manus 在每个关键节点明确告诉你：**当前所处菜单、需要替换的字段、哪些内容保持不变、下一条直接发什么**。只要做到这一点，整套流程就会从“背模板”变成“按提示换挡”。

| 触发时机 | 你对 Manus 说什么 | Manus 必须回报的“应更换点” | 你下一步做什么 |
|------|----------|------------------|----------|
| **还在菜单2抽卡，尚未稳定** | `我现在只是测试抽卡，还没准备切菜单3。` | 明确保持在 `菜单2-B`；提示你继续补参考图、风格方向或资产类型；不得提前登记量产 LoRA | 继续抽卡，不进入量产 |
| **已经抽到满意风格图，准备炼 LoRA** | `我已经抽到约 30 张满意图，准备进入菜单2.5。` | 把当前位点切到 `菜单2.5`；告诉你要准备的训练包、Civitai 参数与测试词，不输出菜单3工单 | 去训练平台炼全局 Style LoRA |
| **LoRA 文件已下载到本地** | `LoRA 已就绪，文件名是 XXX，触发词是 XXX。` | 明确把 `量产默认 LoRA`、`LoRA 触发词`、`菜单3 当前底座` 三个槽位从“未设置”改成具体值 | 完成换轨登记，准备发首个菜单3工单 |
| **准备做第一份菜单3测试资产** | `请给我菜单3的首个测试工单。` | 指定你应使用 `静态资产版 / 动画序列版 / 透视场景版` 哪个分流模板，并告诉你本轮工单要填的资产需求槽位 | 复制对应模板继续下发需求 |
| **菜单3已经跑图，但结果有问题** | `菜单3反馈，第N轮，现象是 XXX。` | 不要整份重写教程；只告诉你哪些节点字段要替换、哪些字段保持不变，以及为什么触发这次熔断调整 | 只修改指定节点，再跑下一轮 |
...
> `【本次要替换的槽位】`
>
> `- 量产 LoRA：未设置 → trickster_style_e10.safetensors`
>
> `- LoRA 触发词：未设置 → trickster_style`
>
> `- 默认分流模板：通用模板 → 动画序列版`
>
> `【保持不变】Base Model 继续沿用 SDXL 当前量产底座；实体蓝图库与知识抽屉不需重写。`
>
> `【请直接复制这条继续】Manus，继续菜单3-动画序列版。资产需求：主角跑动测试 8 帧。请基于当前量产 LoRA 给我下发首份可复制工单。`

> **一句话理解**：菜单2.5负责把“画风”烧进 LoRA；菜单3负责把“需求”翻译成节点。两者之间真正的桥，不只是口令，而是 **“当前菜单判定 + 槽位替换清单 + 下一条可复制消息”**。
...
### 3.4 对话式返工：让 Manus 只告诉你“哪一格要改”

如果你只是想把流程跑通，那么拿到菜单3工单后的最佳协作方式，不是每次都让 Manus 重写整份工单，而是要求它输出**差量修改单**。也就是说，Manus 应该告诉你：哪些节点字段继续沿用，哪些字段需要“替换前 → 替换后”，以及这次修改对应的是哪一种失败现象。这样你在本地 ComfyUI 中只改必要的那几格，测试节奏会快很多。

| 常见失败现象 | 你怎么回报 | Manus 应优先告诉你的替换点 |
|------|------|------|
| **静态资产轮廓发虚、边缘塌陷** | `菜单3反馈，第2轮。静态地刺边缘太软，像糊掉了。` | 优先检查 `ControlNet 类型`、`Canny/Lineart 是否启用`、`KSampler CFG` 与锐边相关负面词 |
| **动画序列滑步、骨折、帧间漂移** | `菜单3反馈，第3轮。主角跑动有滑步，第4帧骨盆位置飘了。` | 优先替换 `DWPose/OpenPose 策略`、`Img2Img Denoising Strength`、序列一致性相关参数 |
| **场景透视崩坏、平台不可落脚** | `菜单3反馈，第2轮。平台厚度乱了，地平线倾斜。` | 优先替换 `Depth/Canny 组合`、透视抽屉调用、结构锁定强度 |
| **画风不稳、像又退回菜单2垫图味道** | `菜单3反馈，第1轮。风格不够统一，像没吃到 LoRA。` | 优先核查 `Load LoRA 权重`、`LoRA 触发词`、是否错误混入了菜单2式的长描述 Prompt |

### 💬 工单返工时，建议固定这样发

> **"Manus，菜单3反馈，第【待你替换】轮。资产：【待你替换】。现象：【待你替换】。请不要整份重写工单，只用【保持不变 / 需要替换 / 替换前→替换后 / 为什么这次改这里】四段式告诉我该改哪几个节点字段。"**

> **建议 Manus 的返工回复长这样：**
>
> `【保持不变】Load LoRA 节点、实体蓝图词保持不变。`
>
> `【需要替换】ControlNet 节点、KSampler 节点。`
>
> `【替换前 → 替换后】Denoising Strength 0.55 → 0.32；ControlNet 从空 → DWPose。`
>
> `【为什么这次改这里】当前失败属于“动画滑步优先级”，按熔断原则先保运动正确，再保材质。`
