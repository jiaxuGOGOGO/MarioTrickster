# 🎨 MarioTrickster 工业化美术管线操作菜单 (V4.1 效率优化版)
...
4. **提示词接力棒必须拆成四层理解**：菜单2.5和菜单3里出现的“提示词”不是同一个东西，不能混用。

| 层级 | 出现位置 | 谁填写 | 作用 | 是否直接喂给最终生图 |
|------|----------|--------|------|----------------------|
| **A. 训练测试词** | Civitai 的 `Sample Media Prompts` | **你** | 只用于训练平台在每个 Epoch 抽样预览 LoRA 是否学到画风 | **否** |
| **B. 换轨口令** | 发给 Manus 的一句话 | **你** | 告诉 Manus：LoRA 文件已落地，本项目正式从菜单2切到菜单3 | **否** |
| **C. 资产需求口令** | 菜单3的需求模板 | **你** | 只描述“这次要做什么资产、静态还是动画、是否要透视/动作约束” | **否** |
| **D. 最终生图 Prompt** | Manus 下发工单里的 `Text Prompt` 节点 | **Manus** | 把 `LoRA 触发词 + 实体蓝图词 + 技法抽屉词 + 反向防错词` 组装成真正喂给模型的极简 Tags | **是** |

> **记忆口诀**：`菜单2.5 的训练测试词 ≠ 发给 Manus 的换轨口令 ≠ 菜单3的资产需求口令 ≠ 最终生图 Prompt`。
>
> 真正进入出图节点的，永远只有最后一层 **D**。前面三层都只是“管线控制语”。

### 3.1 菜单2.5 → 菜单3 的接棒公式

| 阶段 | 你说什么 | Manus/系统做什么 | 最终落点 |
|------|----------|------------------|----------|
| **炼丹期** | 在 Civitai 填 `Sample Media Prompts` | 平台用测试词抽样，帮助你观察 LoRA 是否吃到画风 | **训练平台内部预览** |
| **换轨期** | 告诉 Manus：`LoRA 文件名 + 触发词 + 请切菜单3` | Manus 记住量产底座，不再沿用菜单2的 IPAdapter 垫图逻辑 | **Load LoRA 节点配置基座** |
| **量产期** | 只提本次资产需求 | Manus 从 `PROMPT_RECIPES.md` 抽取实体蓝图 + 技法抽屉 | **工单骨架** |
| **执行期** | 不再自己写长 Prompt | Manus 把最终生图 Tags 分发到 `Load LoRA / Text Prompt / ControlNet / KSampler` | **ComfyUI 节点实际输入** |

> **一句话理解**：菜单2.5负责把“画风”烧进 LoRA；菜单3负责把“需求”翻译成节点。两者中间的桥就是 **LoRA 文件名 + 触发词 + 双轨发牌规则**。
