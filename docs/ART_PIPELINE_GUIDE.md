...
> 4. 冲突熔断原则：当规则冲突时，永远遵循「物理防滑步 > 空间透视正确 > 光影材质表现」的优先级。
...
把找来的杂乱教程扔给 AI，它会自动将知识分拣进不同的"技法抽屉"。

**💬 怎么找 Manus (直接复制发给它)：**
> "Manus，我刚上传了一份顶级的 [美术教程/设定集]。请启动『领域精细化无损蒸馏』：
> 1. 判断核心价值，严格归入 5 个「知识抽屉」的 1-2 个：🧍‍♂️[解剖与形态] / 📐[透视与物件] / 🏃[动画与物理] / 🎨[光影与材质] / ⚙️[AI 硬核参数]。
> 2. 只提炼能生效的 Tag、数值和节点约束。绝不写散文。
> 3. 将提炼的规则，更新到 `prompts/PROMPT_RECIPES.md` 上半部的对应抽屉中。如果不涉及某领域，绝对不要碰那个抽屉！"
