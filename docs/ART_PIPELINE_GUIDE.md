...
> **"Manus，我现在走菜单2-测试模式。目标不是一次定稿，而是先把流程跑通。我会上传参考图或说明需求，你先按最小可执行方案给我下发工单；如果信息不够，只追问会阻塞执行的必要项。"**

### 💬 菜单2固定口令（生产模式）

> **"Manus，我现在走菜单2-生产模式。我要为项目确立可量产的基准画风/实体蓝图。请按正式生产标准给我下发工单：优先保证风格统一、结构稳定、后续可训练 LoRA 或批量复用；如果缺关键信息，只追问必要项。"**
...
> **"Manus，LoRA 已就绪。文件名为 `【你的 LoRA 文件名】`，触发词为 `【你的触发词】`。请把它登记为项目当前默认量产 LoRA，并直接给我下发菜单3的首份测试工单。后续我只说资产需求，你负责按这套量产底座出工单。"**
...
> **"Manus，菜单3反馈。资产是【你的资产名】；当前问题是【你的现象描述】。请按当前量产底座直接重发一份完整的修正版工单；如果有必要，再附一个最多 3 条的关键改动摘要。"**
