...
上一版把菜单2写得过于强调“补位”“换槽位”，确实会让操作心智变重。这里统一收回为更稳定的做法：**菜单2仍然是固定入口，但每次只需先声明你现在是“测试模式”还是“生产模式”**。这样既不需要你背一堆替换规则，也能兼顾快速跑通与项目长期质量基准。

| 模式 | 适合什么时候用 | Manus 应做什么 | 你的目标 |
|------|------|------|------|
| **测试模式** | 你现在只是想先跑通流程、验证节点、看风格方向是否可行 | 优先给出最小可执行工单，默认只保证“流程通、风格方向对、结构不崩” | 用最低沟通成本验证管线 |
| **生产模式** | 你准备把结果纳入项目资产库，要求后续可持续量产 | 优先给出正式规格工单，明确 LoRA、ControlNet、参数安全区间与返工标准 | 保证资产质量、统一性与复用性 |

> **记住一个最简单的原则：**
>
> 菜单2不是让你一次说完所有信息，而是让你用**固定口令**先开局，再由 Manus 只追问真正会卡住执行的关键缺项。不会阻塞执行的内容，先不补。

### 💬 菜单2固定口令（测试模式）

> **"Manus，我现在走菜单2-测试模式。目标不是一次定稿，而是先把流程跑通并验证当前管线可执行。我这次要做的是【角色 / 陷阱 / 交互物 / 场景】中的【填写项】。我这边【有 / 没有】参考图；如果有，我会上传参考图、现有素材或期望风格说明。请你先按最小可执行方案给我下发工单，优先保证流程能顺利跑通；如果信息不够，只追问那些会阻塞执行的必要项，不要一开始把所有细节都展开。"**

### 💬 菜单2固定口令（生产模式）

> **"Manus，我现在走菜单2-生产模式。我要做的是【角色 / 陷阱 / 交互物 / 场景】中的【填写项】。这次目标不是单次测试，而是产出可进入项目基线、后续可量产复用的正式资产。我这边【有 / 没有】参考图；如果有，我会上传参考图、现有素材、风格方向或约束条件。请你按正式生产标准给我下发工单，优先保证风格统一、结构稳定、后续可训练和可复用；如果仍缺信息，只追问那些会影响结果质量或阻塞执行的必要项。"**
...
| 场景 | 极简短句 |
|------|------|
| **菜单2测试** | `我现在走菜单2-测试模式，要做【对象】；有无参考我随后补。先按最小可执行方案下工单，只追问阻塞项。` |
| **菜单2生产** | `我现在走菜单2-生产模式，要做【对象】；目标是项目可量产复用。请按正式生产标准下工单，只追问关键缺项。` |
...
| **已经知道要做什么且手上有参考** | 直接发“菜单2完整版口令”，把对象与参考情况揉进一句话 |
| **当前只是测试跑通流程** | 只要说清“做什么 + 测试模式 + 有无参考”就够了 |
| **当前已经进入正式资产生产** | 在菜单2就明确说明“要量产复用”，避免后面返工时基准不稳 |
...
> **测试模式推荐发法**：
>
> **"Manus，我现在走菜单2-测试模式。目标不是一次定稿，而是先把流程跑通并验证当前管线可执行。我这次要做的是【填写对象】。我这边【有 / 没有】参考图；如果有，我会上传。你先按最小可执行方案给我下发工单；如果信息不够，只追问会阻塞执行的必要项。"**
>
> **生产模式推荐发法**：
>
> **"Manus，我现在走菜单2-生产模式。我要做的是【填写对象】。这次目标不是单次测试，而是产出可进入项目基线、后续可量产复用的正式资产。我这边【有 / 没有】参考图；如果有，我会上传参考图、现有素材或风格约束。请按正式生产标准给我下发工单；如果仍缺信息，只追问会影响质量或阻塞执行的必要项。"**

> 你以后可以只改【对象】和【有/没有参考】这两处，其他句子长期固定使用。这样既兼顾测试效率，也能守住正式生产的质量基准。
>
...
### 3.1 菜单2.5 → 菜单3 的接棒公式（简化版）

菜单2.5 到菜单3不需要你记“换点表”或“槽位清单”。**真正需要你明确告诉 Manus 的，只有两件事：LoRA 文件名是什么，触发词是什么。** 只要这两个量产基座固定下来，菜单3后面就重新回到固定模板驱动。

| 阶段 | 你负责说清楚什么 | Manus 负责做什么 | 最终结果 |
|------|------|------|------|
| **菜单2抽卡期** | 你还在测试风格，暂未进入量产 | 继续给测试工单，不提前切量产 | 先拿到满意风格样本 |
| **菜单2.5炼 LoRA** | 你已经有 30 张满意样本，准备训练 | 给训练参数与测试词建议 | 产出全局 Style LoRA |
| **菜单3换轨登记** | 告诉 Manus：`LoRA 文件名 + 触发词` | 把它登记为量产默认底座 | 后续所有菜单3工单都基于同一标准发牌 |

> **一句话理解**：菜单2.5负责把画风固化成 LoRA；菜单3负责基于这套 LoRA 量产资产。中间不需要你背复杂规则，只需要完成一次**LoRA 登记**。

### 💬 菜单2.5 → 菜单3 固定接棒口令

> **"Manus，菜单2.5已经完成。现在进入菜单3。当前可用的 LoRA 信息如下：文件名是 `【填写文件名】`，触发词是 `【填写触发词】`。请你在不偏离前序目标的前提下，把这些信息并入正式出图工单；如果还缺会影响菜单3执行的必要项，再单独指出。"**

### 菜单2.5 → 菜单3 最短可复制版

| 场景 | 极简短句 |
|------|------|
| **LoRA 已训练完成，准备切量产** | `菜单2.5已完成，LoRA 文件名【X】，触发词【Y】；请并入菜单3正式工单。` |

> **最少原则**：菜单2.5 到菜单3只补 `LoRA 文件名 + 触发词` 两项即可，不需要在这里重新讲一遍完整资产需求。资产要做什么，留到菜单3模板里再说。
