...
## 🏗️ 1. 关卡极速构建流 (Quick Start)

通过 `Level Studio`，策划可以像搭积木一样快速构建并验证关卡。`Level Design` 页签从上到下依次排列着以下区块：

| 区块 | 位置 | 用途 |
|------|------|------|
| **★ Custom Template Editor** | 页面最顶部 | 片段选择 + 元数据预览 + ASCII 文本编辑 + 生成 |
| **Element Palette** | 中部 | 点击单个元素直接放到 Scene 中心 |
| **★ Gameplay Mechanics** | 中部 | 附身点网络 / 路线预算 / 机制验证 / Auto-Fix |
| **Quick Whitebox Generator** | 中下部 | 从内置模板一键生成完整白盒 |

<details open>
<summary><b>🕹️ 操作步骤详解 (点击折叠/展开)</b></summary>
...
- **方式 B（自由编辑）**：在下方的大文本框 `模板内容` 中手动编写或粘贴 ASCII 模板，然后点击绿色的 **`[Build From Text (生成关卡)]`** 按钮。
- **方式 C（内置模板快捷）**：滚动到页面中下方的 `Quick Whitebox Generator` 区块，从 `Template` 下拉菜单选择一个内置模板，点击绿色的 **`[Generate Whitebox Level]`** 按钮。
...
5. **导出战报**：挂机一段时间后，点击黄色的 `[Print Match Report]` 按钮。系统会在 `docs/validation/` 目录下生成结构化战报和摘要。

</details>

<details>
<summary><b>💡 LLM 调优提示词模板示例 (点击展开)</b></summary>
...
<details open>
<summary><b>📝 交互黑匣子 (Interaction Log)</b></summary>
...
<details open>
<summary><b>🎯 智能降噪可视化 (Gameplay Box Visualizer)</b></summary>
