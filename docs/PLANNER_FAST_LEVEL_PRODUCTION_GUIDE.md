...
| 素材包批量整理 | 已优化 | `Ctrl+T → Art & Effects Hub → 策划生产助手` | 选择素材文件夹，生成语义报告或自动填主题 | 扫描 Sprite、分类、建议命名、匹配 Theme Profile 槽位 |
...
| 素材包命名混乱 / 主题槽太多 | 策划生产助手 | 选文件夹和 Theme，点语义报告或自动填槽 | 输出建议命名并自动绑定可识别的主题 Sprite |
...
| --- | --- | --- | --- |
| **商业素材包命名混乱** | 分类准确率下降 | 素材包语义巡检 + 建议命名报告 | 选择素材文件夹，点“生成语义报告”，系统列出分类、动画、推荐槽位和建议文件名 |
...
| 角色素材只覆盖 idle/run/jump/fall 主状态 | attack/hurt/death 尚未全自动 | 暂保持按需扩展 | 等实际角色资产出现 attack/hurt/death 需求时，再沿 `SpriteStateAnimator` 扩状态，避免提前复杂化 |
| 自动化测试无法在沙盒跑 Unity | 需要用户本地 Unity 重跑 | 测试报告继续驱动修复 | 本地 Unity 导出报告后，AI 按报告全局修复并推送 |

策划生产助手的原则是“先安全辅助，再自动落地”。它不会强行批量改坏原始素材引用，而是先产出语义报告与建议命名；它会自动填 Theme Profile 中能明显识别的槽位，但不会清空你已经确认过的插槽；它也不会把 Unity 编辑器变成不稳定的自动改代码黑盒，而是把机制请求整理成外部更强 AI 可以直接执行的标准任务。
