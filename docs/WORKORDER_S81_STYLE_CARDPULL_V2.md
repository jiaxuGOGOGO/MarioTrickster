...
### 7. 核心采样器
**节点名**：`K采样器` (KSampler)
- **种子 (seed)**：保持默认的 `0` 不用改（这是一个纯数字输入框，没有“随机”选项）
- **生成后控制 (control_after_generate)**：保持默认的 `randomize`（这个才是“随机”的开关：每次跑完一张图后，自动把种子换成新的随机数）
  - *复现好图方法：把 `生成后控制` 改为 `fixed`，然后把好图对应的种子数字填回 `种子` 框里。*
